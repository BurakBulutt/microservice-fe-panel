...
import {FaAnchor, FaImage, FaPlus, FaTrash} from "react-icons/fa";
...
const ContentMeta = (props) => {
  ...
  const handleSubmit = () => {
    formik.setFieldValue("categoryIds",formik.values?.categories?.map(item => item.id));
    formik.handleSubmit();
  }

  const handleSelect = (data) => {
    if (!formik.values?.categories?.map(item => item.id).includes(data.id)) {
    ...
    <div className="flex items-start space-x-4">
      ...
      <div className="mb-4 flex flex-col w-full">
        <div className="mb-4 flex flex-row justify-between space-x-4">
          ...
          <div className="flex-1">
            ...
            {formik.errors.startDate && (
                <div className="ml-2 mt-2 text-red-500">{formik.errors.startDate}</div>
            )}
          </div>
          <div className="flex-1">
            <InputField
          ...
          <div className="flex-1">
            ...
            <select
                className="mt-2 flex h-12 w-full items-center justify-center rounded-xl border bg-white/0 p-3 text-sm outline-none border-gray-200 dark:border-white/10 dark:text-white focus:border-blue-500 dark:focus:border-blue-400"
          ...
          <div className="flex-1">
            ...
            {formik.errors.episodeTime && (
                <div className="ml-2 mt-2 text-red-500">{formik.errors.episodeTime}</div>
            )}
          </div>
        </div>
        ...
        <div className="mb-4 flex flex-col space-y-4">
          ...
          <div className="w-full overflow-hidden">
            <button className="w-full rounded-xl bg-green-500 px-5 py-3 text-base font-medium text-white transition duration-200 hover:bg-green-600 active:bg-green-700 dark:bg-green-400 dark:text-white dark:hover:bg-green-300 dark:active:bg-green-200"
